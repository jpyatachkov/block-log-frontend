...
function makeTest(testsArray, fileContent) {
  console.log('111111');
  const bodyObject = {
    testsArray: testsArray,
    fileContent: fileContent,
  };
  function testing(bodyObject) {
    console.log('22222222');
    let AAAA_TEST_GLOBAL_RESULT = '';
    let response = {
      end: (s) => {
        AAAA_TEST_GLOBAL_RESULT = s + '';
      },
    };
    // work with object
    function workWithObject(bodyObject, response) {
      ...
      for (let i = 0; i < TESTS_MAIN_ARRAY.length; i++) {
        ...
        getId(
          'consoleBoxContentValuesBoxForInputData',
        ).value = TEST.inputArray.join(' ');
        getId('testingOutputBox').innerHTML +=
          'Input: ' + TEST.inputArray.join(' ') + '<br>';
        let obj = null;
        try {
        ...
        if (outputString === TEST.outputArray.join('  ') + '  ') {
          getId('testingOutputBox').innerHTML +=
            'Right output: ' + TEST.outputArray.join('  ') + '<br>';
          getId('testingOutputBox').innerHTML +=
            'Your output: ' + outputString + '<br>';
      ...
      /// /
      console.log('----------------------------------');
      const RESPONSE_TEXT = (getId('testingOutputBox').innerHTML + ' ')
        .split('<br>')
        .join('\n')
        .split('  ')
        .join(' ');
    ...
    function testWork(objParam, getId) {
      ...
      class ProgramRunner {
        ...
        static getHitBlockPoint(xxx, yyy, b) {
          if (ProgramRunner.getHitBlockTochka(xxx, yyy, b) === true)
            return true;
          if (ProgramRunner.getHitBlockTochka(xxx - 10, yyy - 10, b) === true)
            return true;
          if (ProgramRunner.getHitBlockTochka(xxx + 10, yyy + 10, b) === true)
            return true;
          if (ProgramRunner.getHitBlockTochka(xxx + 10, yyy - 10, b) === true)
          ...
          while (true) {
            ...
            if (workOutput !== CODE_OK && workOutput !== CODE_ERROR) {
              ...
              for (let i = 0; i < arr.length; i++) {
                const b = arr[i];
                if (
                  ProgramRunner.getHitBlockPoint(
                    nowBlock.strFinishX,
                    nowBlock.strFinishY,
                    b,
                  )
              ...
              for (let i = 0; i < arr.length; i++) {
                ...
                if (workOutput === CODE_OK) {
                  ...
                  if (
                    ProgramRunner.getHitBlockPoint(
                      nowBlock.strFinishXsecond,
                      nowBlock.strFinishYsecond,
                      b,
                    )
                  ) {
                    nowBlock = b;
        ...
        getTableCode() {
          ...
          tableCode =
            tableCode +
            '<tr><td><b>Название</b></td><td><b>Количество</b></td></tr>';
          tableCode =
            tableCode +
            ('<tr><td>Базовый</td><td>' +
              this.operationsCountObj.base +
    ...
    return AAAA_TEST_GLOBAL_RESULT + '';
  }
  //
  //
  return testing(bodyObject);
}

self.onmessage = (message) => {
  const { testsArray, fileContent } = message.data;
  const answerText = makeTest(testsArray, fileContent);
  postMessage({
    programIsCorrect: answerText.indexOf('ERROR') === -1,
  });
};
